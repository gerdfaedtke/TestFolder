...
class StopWatch: ObservableObject {
    ...
    func lap() {
        if let firstLap = self.laps.first {
            ...
            self.currentLaps.append(LapItem(count: self.counter, difference: difference))
        } else {
...
extension StopWatch {
    struct LapItem {
        ...
        init(count: Int, difference: Int = -1) {
            self.count = count
            
            if difference < 0 {
                self.stringTime = StopWatch.convertCountToTimeString(counter: count)
            } else {
                self.stringTime = StopWatch.convertCountToTimeString(counter: difference)
            }
        }
    }
}
